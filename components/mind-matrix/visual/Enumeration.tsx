            ...
            <InstructionDialog
                ...
                instructions={
                    <div className="text-center space-y-4">
                        <p className="text-xl">You will see a grid of two soft colors, rose and sky.</p>
                        <p className="text-xl font-semibold text-rose-800">Count the rose-colored squares carefully.</p>
                    ...
                    {Array.from({ length: grid_size * grid_size }).map((_, i) => {
                        ...
                        return (
                            <div key={i} className="w-full h-full flex items-center justify-center">
                                {dot && (
                                    <div
                                        className={`w-4/5 h-4/5 rounded-sm shadow-sm ${dot.color === 'red' ? 'bg-rose-400' : 'bg-sky-400'}`}
                                    />
                                )}
                            </div>
                        );
    ...
    if (showOptions) {
        return (
            <div className="flex flex-col items-center justify-center p-8">
                <h3 className="text-xl font-bold mb-6">How many rose-colored squares?</h3>
