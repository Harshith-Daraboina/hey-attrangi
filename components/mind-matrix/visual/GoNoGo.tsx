...
export default function GoNoGo({ question, onComplete }: GoNoGoProps) {
    ...
    const resultsRef = useRef<{ type: string; response: boolean }[]>([]);
            ...
            <InstructionDialog
                ...
                instructions={
                    <div className="text-center space-y-4">
                        <p className="text-xl">Press <strong className="bg-stone-200 px-2 py-1 rounded">SPACE</strong> when you see the <span className="text-emerald-700 font-bold">teal</span> circle.</p>
                        <p className="text-xl">Do <strong className="text-stone-700 font-bold">not</strong> press for the <span className="text-amber-700 font-bold">amber</span> circle.</p>
    ...
    return (
        <div className="flex flex-col items-center justify-center p-8 h-96">
            ...
            {gameState === 'STIMULUS' && (
                ...
                ${stimulusType === 'GO' ? 'bg-emerald-500' : 'bg-amber-500'}
            `}
                />
            )}

            {gameState === 'FINISHED' && (
                <div className="text-lg font-medium text-stone-700">Almost there…</div>
            )}
        </div>
    );
}
