...
// Helper function to extract subquestions from reflection questions text
function extractSubquestions(text: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed && (trimmed.startsWith('Have') || trimmed.startsWith('Do') || trimmed.startsWith('Did') || trimmed.startsWith('Was') || trimmed.startsWith('Which') || trimmed.startsWith('What') || trimmed.startsWith('When') || trimmed.startsWith('Are') || trimmed.startsWith('Could') || trimmed.startsWith('Am'))) {
...
async function main() {
  try {
    console.log("Starting to insert blog entries...\n");

    for (let i = 0; i < blogs.length; i++) {
      const blogData = blogs[i];
      const slug = createSlug(blogData.title);

      console.log(`Creating blog ${i + 1}/${blogs.length}: ${blogData.title}`);
      ...
      console.log(`✓ Blog created/updated: ${blog.title} (ID: ${blog.id})\n`);
    }

    console.log(`\n✅ Successfully processed ${blogs.length} blog entries!`);
  } catch (error) {
