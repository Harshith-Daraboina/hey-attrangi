...
// Types matching the Prisma schema/response
interface Blog {
    id: string;
    title: string;
    slug: string;
...
export default function BlogClient({ initialBlog, recentBlogs }: BlogClientProps) {
    const [blog, setBlog] = useState<Blog>(initialBlog);
    const [reviews, setReviews] = useState<Review[]>(initialBlog.reviews || []);
    ...
    useEffect(() => {
        ...
        if (initialBlog?.id) {
            fetch(`/api/blogs/${initialBlog.id}/view`, {
                method: "POST",
    ...
    const handleReviewSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/blogs/${blog.id}/review`, {
                method: "POST",
                headers: {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <Navigation currentPath={`/blogs/${blog.id}`} />
                            ...
                            <div className="p-8 md:p-12">
                                ...
                                <div className="flex items-center gap-4 py-6 border-t border-gray-200">
                                    <button
                                        onClick={handleLike}
                                        disabled={liked}
                                        className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-colors ${liked
